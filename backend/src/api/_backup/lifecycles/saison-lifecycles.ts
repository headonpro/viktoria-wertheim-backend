...
async function validateSeasonDates(data: any) {
  if (data.start_datum && data.end_datum) {
    ...
    // Temporarily disable overlap validation to allow season setup
    // TODO: Re-enable after all seasons are properly configured
    strapi.log.info('Season date overlap validation temporarily disabled');
