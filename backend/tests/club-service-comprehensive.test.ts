...
const mockLigen = [
  { id: 1, name: 'Kreisliga Tauberbischofsheim', aktiv: true },
  { id: 2, name: 'Kreisklasse A Tauberbischofsheim', aktiv: true },
  { id: 3, name: 'Inactive Liga', aktiv: false }
];
...
describe('Club Service - Comprehensive Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Create club service instance with mocked dependencies
    clubService = factories.createCoreService('api::club.club', () => mockStrapi)({
      strapi: mockStrapi
    });

    // Mock cache manager initialization
    clubService._cacheManager = mockCacheManager;
  });
  ...
  describe('Validation Logic', () => {
    describe('validateClubData', () => {
      ...
      it('should handle validation errors gracefully', async () => {
        ...
        // Force an error by making the validation throw
  ...
  describe('Error Handling', () => {
    it('should handle database connection errors', async () => {
      ...
      await expect(clubService.findClubsByLiga(1)).rejects.toThrow(
        'Failed to find clubs for liga 1: Connection timeout'
      );
      expect(mockStrapi.log.error).toHaveBeenCalledWith('Error finding clubs by liga:', dbError);
    });

    it('should handle validation system errors', async () => {
      const validationError = new Error('Validation system failure');
      mockStrapi.entityService.findOne.mockRejectedValue(validationError);

      const result = await clubService.validateClubConsistency(1);

      expect(result.isValid).toBe(false);
      expect(result.errors[0]).toContain('Validation error: Validation system failure');
      expect(mockStrapi.log.error).toHaveBeenCalledWith('Error validating club consistency:', validationError);
    });

    it('should handle cache errors gracefully', async () => {
      const cacheError = new Error('Cache connection failed');
      mockCacheManager.getClubsByLiga.mockRejectedValue(cacheError);
      mockStrapi.entityService.findMany.mockResolvedValue([mockClubs[0]]);

      const result = await clubService.findClubsByLiga(1);

      expect(mockStrapi.log.warn).toHaveBeenCalledWith('Cache error, falling back to database:', cacheError.message);
      expect(mockStrapi.entityService.findMany).toHaveBeenCalled();
      expect(result).toEqual([mockClubs[0]]);
    });

    it('should handle entity service errors', async () => {
      const entityError = new Error('Entity service unavailable');
      mockStrapi.entityService.create.mockRejectedValue(entityError);
      mockStrapi.entityService.findMany.mockResolvedValue([]); // No existing clubs

      clubService.validateClubData = jest.fn().mockResolvedValue({ isValid: true, errors: [] });

      const clubData = { name: 'Test Club', club_typ: 'gegner_verein' as const };

      await expect(clubService.createClubIfNotExists(clubData)).rejects.toThrow(
        'Failed to create club Test Club: Entity service unavailable'
      );
    });
  });
  ...
  describe('Performance and Edge Cases', () => {
    it('should handle large datasets efficiently', async () => {
      const largeClubSet = Array.from({ length: 1000 }, (_, i) => ({
        ...
        id: i + 1,
        name: `Club ${i + 1}`,
        viktoria_team_mapping: i < 3 ? `team_${i + 1}` as const : undefined
      }));

      mockStrapi.entityService.findMany.mockResolvedValue(largeClubSet);
      ...
      expect(result.summary.totalClubs).toBe(1000);
    });
    ...
    it('should handle empty database gracefully', async () => {
      ...
      const validationResult = await clubService.validateAllClubData();
      expect(validationResult.isValid).toBe(true);
      expect(validationResult.summary.totalClubs).toBe(0);
    });
